...
Public Class SensusForm
    ...
    Dim ruangan, kelas, ruanganParam, kelasParam, sip As String
    ...
    Private Sub btnCari_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnCari.Click
        ...
        If conn.State = ConnectionState.Closed Then
            conn.Open()
            query = "SELECT SUM(awal) AS [Pasien awal], SUM(masuk) AS [Pasien masuk], SUM(pindahan) AS [Pasien pindahan], SUM(diizinkan) AS [Diizinkan pulang], SUM(rujuk) AS [Dirujuk], SUM(pindah) AS [Pindah RS lain], SUM(paksa) AS [Pulang paksa], SUM(melarikan) AS [Melarikan diri], SUM(dipindah) AS [Dipindahkan], SUM(kurang) AS [Meninggal <48 jam], SUM(lebih) AS [Meninggal >48 jam], SUM(lama) AS [Lama dirawat], SUM(keluarmasuk) AS [Keluar Masuk] FROM ( SELECT COUNT(register_masuk.id) AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, 0 AS lama, 0 AS keluarmasuk FROM (ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur] WHERE tanggal_masuk < @tgl AND register_masuk.deleted_at IS NULL AND keluar = FALSE AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT COUNT(register_masuk.id) AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, 0 AS lama, 0 AS keluarmasuk FROM ((ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur]) INNER JOIN register_keluar ON register_masuk.[id] = register_keluar.[id_register_masuk] WHERE register_masuk.deleted_at IS NULL AND register_keluar.deleted_at IS NULL AND keluar = TRUE AND tanggal_keluar > @tgl AND tanggal_masuk < @tgl AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, SUM(IIF([cara_pasien_masuk] <> 'Pindahan', 1, 0)) AS masuk, SUM(IIF([cara_pasien_masuk] = 'Pindahan', 1, 0)) AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, 0 AS lama, 0 AS keluarmasuk FROM (ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur] WHERE tanggal_masuk = @tgl AND register_masuk.deleted_at IS NULL AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, SUM(IIF([cara_pasien_keluar] = 'Diizinkan pulang', 1, 0)) AS diizinkan, SUM(IIF([cara_pasien_keluar] = 'Dirujuk', 1, 0)) AS rujuk, SUM(IIF([cara_pasien_keluar] = 'Pindah RS lain', 1, 0)) AS pindah, SUM(IIF([cara_pasien_keluar] = 'Pulang paksa', 1, 0)) AS paksa, SUM(IIF([cara_pasien_keluar] = 'Melarikan diri', 1, 0)) AS melarikan, SUM(IIF([cara_pasien_keluar] = 'Dipindahkan', 1, 0)) AS dipindah, SUM(IIF([cara_pasien_keluar] = 'Meninggal <48 jam', 1, 0)) AS kurang, SUM(IIF([cara_pasien_keluar] = 'Meninggal >48 jam', 1, 0)) AS lebih, 0 AS lama, 0 AS keluarmasuk FROM ((ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur]) INNER JOIN register_keluar ON register_masuk.[id] = register_keluar.[id_register_masuk] WHERE tanggal_keluar = @tgl AND register_keluar.deleted_at IS NULL AND register_masuk.deleted_at IS NULL AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, SUM(DATEDIFF('y', [tanggal_masuk], @tgl)) AS lama, 0 AS keluarmasuk FROM (ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur] WHERE register_masuk.deleted_at IS NULL AND keluar = FALSE AND tanggal_masuk < @tgl AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, COUNT(register_masuk.id) AS lama, 0 AS keluarmasuk FROM (ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur] WHERE register_masuk.deleted_at IS NULL AND tanggal_masuk = @tgl AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, SUM(DATEDIFF('y', [tanggal_masuk], @tgl)) AS lama, 0 AS keluarmasuk FROM ((ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur]) INNER JOIN register_keluar ON register_masuk.[id] = register_keluar.[id_register_masuk] WHERE register_masuk.deleted_at IS NULL AND register_keluar.deleted_at IS NULL AND keluar = TRUE AND tanggal_keluar >= @tgl AND tanggal_masuk < @tgl AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, 0 AS lama, COUNT(register_masuk.id) AS keluarmasuk FROM (ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur] WHERE tanggal_masuk = @tgl AND register_masuk.deleted_at IS NULL AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip UNION ALL SELECT 0 AS awal, 0 AS masuk, 0 AS pindahan, 0 AS diizinkan, 0 AS rujuk, 0 AS pindah, 0 AS paksa, 0 AS melarikan, 0 AS dipindah, 0 AS kurang, 0 AS lebih, 0 AS lama, COUNT(register_keluar.id) AS keluarmasuk FROM ((ruangan INNER JOIN (kelas INNER JOIN tempat_tidur ON kelas.[id] = tempat_tidur.[id_kelas]) ON ruangan.[id] = tempat_tidur.[id_ruangan]) INNER JOIN register_masuk ON tempat_tidur.[id] = register_masuk.[id_tempat_tidur]) INNER JOIN register_keluar ON register_masuk.[id] = register_keluar.[id_register_masuk] WHERE tanggal_keluar = @tgl AND register_masuk.deleted_at IS NULL AND register_keluar.deleted_at IS NULL AND ruangan LIKE @ruangan AND kelas LIKE @kelas AND sip_dokter LIKE @sip )  AS [%$##@_Alias];"
